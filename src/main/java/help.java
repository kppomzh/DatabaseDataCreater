public class help {
    static String help="本程序的作用是根据给定的建立数据库表的SQL（create语句），自动的生成给定行数的数据，\n" +
            ...
            "telephone（手机号）、email（电子邮件）、ch_word（汉字字符串）、a/b/c/d/e_ip（五种IP地址）、warp_latitude（经纬度）的随机生成；自定义格式支持以下五种：字母 c、大写字母 b、小写字母 s、数字 n、（特定）标点符号 m\n" +
            ...
            "目前支持通过default关键字设置数值型/字符型字段的默认值，但是不对数据逻辑进行检验，请自行保证正确性。\n"+
