...
1. 下载 https://github.com/kppomzh/DatabaseDataCreater/releases/download/V1.4.1/DBDF-1.4.1.zip  
2. 解压  
3. 命令行定位到jar包所在位置  
4. 输入命令：java -jar DBDF-1.4.1.jar  
...
### 数据导入
toDB:传递给DB的方式，目前允许jdbc（通过jdbc传SQL），sql（写到对应的表名称的sql文件里），csv（通过写入类似外部表的文件载入），json（json字
符串文件）~~，mongo（MongoDB）~~。    
baseFileDir:数据文件写入位置，当toDB为jdbc的时候参数失效。    
canbeNegative:是否允许数值型数据出现负数，当指定输入范围时失效。   
dbCharSet:指定输出的字符串以什么字符集写入指定位置；理论上，Java支持很多种字符集的编解码，但是我在此不可能列出所有的编码方案来，仅列出常见的7
种字符集放在配置文件中供参考。该值可以留空，留空后会选择当前环境中的默认字符集填入；该值区分大小写。 
