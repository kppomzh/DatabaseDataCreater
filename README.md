...
-n:指定每个表输出的数据条数，用逗号进行分隔（如三张表：5,100,100）；如果不指定或者参数少于表个数，将会对每张未指定生成条数的表询问具体的输出条数。  
-o:指定保存输出数据的文件夹位置。  
...
5. 过大的表尽量不要使用unique约束，会导致对数函数级别的性能下降与极大的内存占用。  
6. 本工具不支持过分复杂的create，带有索引或者约束的SQL请将这些限制直接的写在对应字段后方，不要写在create括号的后面，否则会被掠过。  
7. 一次运行尽量不要添加过多的外键约束，尤其是外键约束的主表不要太多，会导致性能小幅下降并且占用更多内存。
